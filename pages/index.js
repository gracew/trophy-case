    ...
    await Promise.all(
      Array.from(contracts).map(async (contract) => {
        const parsed = await fetch(
          `https://deep-index.moralis.io/api/v2/nft/${contract}/owners?chain=mumbai&format=decimal`,
          {
            headers: {
              "X-API-Key": process.env.NEXT_PUBLIC_MORALIS_KEY,
            },
      ...
      Array.from(users)
        ...
        .map(async (user) => {
          ...
          const userContracts = new Set(
            userBadges.map((badge) => badge.token_address)
          );
          const numShared = Array.from(userContracts).filter((i) =>
            contracts.has(i)
          ).length;
        ...
        {address && (
          <div>
            <Nav className={styles.nav} activeKey="badges" onSelect={setTab}>
              <Nav.Item>
                <Nav.Link
                  eventKey="badges"
                  className={
                    tab === "badges" ? styles.activeNavLink : styles.navLink
                  }
                >
                  My Badges
                </Nav.Link>
              ...
              <Nav.Item>
                <Nav.Link
                  eventKey="similar"
                  className={
                    tab === "similar" ? styles.activeNavLink : styles.navLink
                  }
                >
                  Similar to You
                </Nav.Link>
            ...
            <div className={styles.grid}>
              {tab === "badges" &&
                badges
                  .filter(
                    (badge) =>
                      badge.metadata &&
                      badge.metadata.image?.startsWith("https")
                  )
                  .map((badge) => (
                    <a
                      key={badge.token_id}
              ...
              {tab === "similar" &&
                similar.map((similar) => (
                  <User key={similar[0]} address={similar[0]} />
                ))}
            </div>
          </div>
        )}
